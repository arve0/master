...
![The process of segmentation in a graphical user interface. Regions 4-2, 11-7 and 14-1 might be adjusted by the user, all other regions are detected fairly well.](figures/leicaautomator_web.png)

#### Imaging each region
settings, laser, objective, collector, aperture

After regions has been localized, pixel-size in meters is calculated by

$$ x_{resolution} = \frac{\Delta pixels}{\Delta x} $$ {#eq:resolution}

.
The stage position for the center pixel in the top left overview image was read from OME-XML metadata. Left most left pixel is calculated by

$$ X_{start} = x_{center} - \frac{S_x \cdot x_{resolution}}{2} $$ {#eq:firstx}

and then stage x-coordinate was calculated with

$$ X = X_{start} + x \cdot x_{resolution} $$ {#eq:pos}

Equ

By dividing , and the top  A bounding box around each region is calculated and the corresponding stage coordinates. By looping through region row and column in zick-zack, a single scanning template is made for each region (due to a limitation in the Leica LAS software) and scanning is started by communication with the Leica software over the CAM socket interface. The output filename is recorded and when the scanning of each region is done, the images are rotated 270 degrees, so that x- and y-axis are the same in the images as the microscope stage is.

#### SHG images
25x water objective and a 0.9 NA air collector was used. The pulsed infrared laser was set to 890 nm, intensity 20%, gain 40%, offset 80% and electro-optic modulator (EOM) on. Forward light was measured with non descanned PMT sensor with 445/20 nm filter and gain adjusted so that signal spanned the whole intensity range.
...
## Segmentation
![Comparison of thresholding](figures/thresholding_web.png)
